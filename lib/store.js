...
import {
  collection, addDoc, setDoc, deleteDoc, doc, updateDoc, query, orderBy, where, getDoc,
...
const getLocalStorageHandlers = (localStorageFallbackKey) => {
  ...
  if (import.meta.env.MODE !== 'test' && !getAuth().user) return null;
...
function createDocActions(docSchema, ensureExists, localStorageFallbackKey) {
  return {
    ...
    async $fetch(id) {
      ...
      if (getLocalItem) return getLocalItem(localStorageFallbackKey);
    ...
    const useStore = defineStore(id, {
      ...
      actions: {
        ...
        async _sync(docData, keys) {
          ...
          if (localStorageHandlers) return localStorageHandlers.set({ ...docData, ...dataToSync });
